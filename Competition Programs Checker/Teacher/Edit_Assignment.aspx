<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Edit_Assignment.aspx.cs" Inherits="Competition_Programs_Checker.Teacher.Edit_Assignment" %>
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    <div class="container">
        <asp:SqlDataSource ID="problem" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" SelectCommand="SELECT * FROM [Problem]
WHERE [id]=@problem_id" InsertCommand="INSERT INTO Problem(code, title, author, assignment, is_active) 
VALUES (@code, @title, @author, @assignment, @is_active);
SELECT @inserted_id = SCOPE_IDENTITY();" OnInserted="GetLastUsedProblemId" DeleteCommand="DELETE FROM Problem
WHERE [id]=@id">
            <DeleteParameters>
        ...
        <asp:SqlDataSource ID="testRuns" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" SelectCommand="SELECT [id], [problem_id], [order_position], [input], [output] FROM [TestRuns]
WHERE [problem_id]=@id">
            <SelectParameters>
                <asp:QueryStringParameter DefaultValue="0" Name="id" QueryStringField="id" />
            </SelectParameters>
